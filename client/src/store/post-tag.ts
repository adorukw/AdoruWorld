...
import { postTagApi as api } from "@/api";
...
export const usePostTagStore = defineStore("post-tag", () => {
  ...
  const getPostTagBySlug = async (slug: string) => {
    ...
    error.value = null;
    try {
      currentPostTag.value = await api.getBySlug(slug);
    } catch (err: any) {
      error.value = err.message || "获取标签失败";
    } finally {
      loading.value = false;
  ...
  const getPostTagById = async (id: string) => {
    ...
    error.value = null;
    try {
      currentPostTag.value = await api.getById(id);
    } catch (err: any) {
      error.value = err.message || "获取标签失败";
    } finally {
      loading.value = false;
