...
import { mediaTagApi as api } from "@/api";
...
export const useMediaTagStore = defineStore("media-tag", () => {
  ...
  const getMediaTags = async () => {
    ...
    error.value = null;
    try {
      mediaTags.value = await api.list();
    } catch (err: any) {
  ...
  const getMediaTagBySlug = async (slug: string) => {
    ...
    error.value = null;
    try {
      currentMediaTag.value = await api.getBySlug(slug);
    } catch (err: any) {
      error.value = err.message || "获取标签失败";
    } finally {
      loading.value = false;
  ...
  const getMediaTagById = async (id: string) => {
    ...
    error.value = null;
    try {
      currentMediaTag.value = await api.getById(id);
    } catch (err: any) {
      error.value = err.message || "获取标签失败";
    } finally {
      loading.value = false;
  ...
  const createMediaTag = async (data: MediaTagCreate) => {
    ...
    error.value = null;
    try {
      const newTag = await api.create(data);
  ...
  const updateMediaTag = async (id: string, data: MediaTagUpdate) => {
    ...
    error.value = null;
    try {
      const updatedTag = await api.update(id, data);
