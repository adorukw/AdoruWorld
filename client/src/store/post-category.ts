...
import { postCategoryApi as api } from "@/api";
import type {
...
export const usePostCategoryStore = defineStore("post-categories", () => {
  ...
  const postCategoryMap = computed(() => {
    return postCategories.value.reduce(
      (map, category) => {
  ...
  const getPostCategories = async () => {
    ...
    error.value = null;
    try {
      postCategories.value = await api.list();
    } catch (err: any) {
  ...
  const getPostCategoryBySlug = async (slug: string) => {
    ...
    error.value = null;
    try {
      currentPostCategory.value = await api.getBySlug(slug);
    } catch (err: any) {
      error.value = err.message || "获取分类失败";
    } finally {
      loading.value = false;
  ...
  const getPostCategoryById = async (id: string) => {
    ...
    error.value = null;
    try {
      currentPostCategory.value = await api.getById(id);
    } catch (err: any) {
      error.value = err.message || "获取分类失败";
    } finally {
      loading.value = false;
  ...
  const createPostCategory = async (data: PostCategoryCreate) => {
    ...
    error.value = null;
    try {
      const newCategory = await api.create(data);
  ...
  const updatePostCategory = async (id: string, data: PostCategoryUpdate) => {
    ...
    error.value = null;
    try {
      const updatedCategory = await api.update(id, data);
      const index = postCategories.value.findIndex(
