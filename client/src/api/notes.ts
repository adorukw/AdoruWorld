...
function sortTree(nodes: NoteTreeNode[]): NoteTreeNode[] {
  const dirs = nodes.filter((n) => n.type === 'dir').sort((a, b) => a.name.localeCompare(b.name, 'zh-CN'))
  const files = nodes.filter((n) => n.type === 'file').sort((a, b) => a.name.localeCompare(b.name, 'zh-CN'))
