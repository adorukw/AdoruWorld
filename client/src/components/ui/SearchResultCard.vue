    ...
    <div>
        <div :to="routerLink"
            ...
            <div class="flex gap-4">
                <!-- 左侧封面区 -->
                <div v-if="item.coverImage" class="w-28 h-28 shrink-0 border-2 border-black overflow-hidden">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <div class="flex items-center gap-2 mb-2">
                        <span class="text-xs px-2 py-0.5 border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,0.5)]"
                            :class="{
                                'bg-blue-500 text-white': item.type === 'post',
                                'bg-red-500 text-white': item.type === 'dex',
                                'bg-green-500 text-white': item.type === 'media',
                            }">
                            {{ categoryIcon }} {{ typeLabel }}
                        </span>
                        <span v-if="categoryName" class="text-xs px-2 py-0.5 bg-gray-100 border-2 border-black">
                            {{ categoryName }}
                        </span>
                    </div>
                    ...
                    <!-- 匹配字段提示（做了高亮） -->
                    <div v-if="item.matchedFields.length > 0" class="flex flex-wrap gap-1 mb-2">
                        <span class="text-xs text-yellow-700 bg-yellow-100 px-1.5 py-0.5">
                    ...
                    <div v-if="tags.length > 0" class="flex flex-wrap gap-1">
                        <span v-for="t in tags.slice(0, 4)" :key="t.id || t.name"
                            class="text-xs px-1.5 py-0.5 bg-gray-50 border border-gray-300">
                            #{{ t.name }}
                        </span>
                        <span v-if="tags.length > 4" class="text-xs text-gray-500">
                            +{{ tags.length - 4 }}
                        </span>
                    </div>
                </div>
