<template>
    ...
    <router-link
        v-if="viewMode === 'grid'"
        :to="`/dex/${entry.slug}`"
        class="group block perspective-box"
    >
        <!-- 3D 盒子容器 -->
        <div
            class="bd-volume relative w-full aspect-11/14 bg-black cursor-pointer"
        >
            <!-- 左侧书脊 (Spine) -->
            <div
            ...
            >
                <span
                    class="text-[6px] text-white/40 font-bold tracking-widest rotate-90 mt-4"
                    >BLU-RAY</span
                >
                <!-- 侧边编号模拟 -->
                <span
                    class="text-[8px] text-white/50 font-mono rotate-90 mb-4"
                    >{{ formattedIndex }}</span
                >
            </div>
            ...
            <!-- 底部边 (Bottom Edge) -->
            <div
                class="bd-bottom absolute bottom-0 left-0 right-0 h-4 origin-bottom bg-blue-950"
            ></div>
            ...
            >
                <!-- 塑封反光层 -->
                <div
                    class="bd-shine absolute inset-0 z-20 pointer-events-none mix-blend-overlay"
                ></div>
                ...
                <div class="bg-black relative flex flex-col h-full w-full">
                    ...
                    >
                        <span
                            class="text-white/90 text-[9px] font-bold tracking-widest uppercase bd-logo-font"
                        >
                            Blu-ray Disc
                        </span>
                    </div>
                    ...
                    <div class="relative flex-1 w-full overflow-hidden">
                        ...
                        >
                            ...
                            <h3
                                class="text-white! font-bold text-sm leading-tight line-clamp-2 mb-2 drop-shadow-lg"
                            >
                                {{ entry.title }}
                            </h3>
    ...
    <router-link v-else :to="`/dex/${entry.slug}`" class="block group">
        ...
        >
            ...
            <div class="perspective-box w-24 shrink-0 py-2 pl-2">
                <div
                    class="bd-volume-list relative w-full aspect-11/14 bg-black"
                >
                    <!-- 缩略图书脊 -->
                    <div
                        class="absolute left-0 top-0 bottom-0 w-2.5 origin-left bg-blue-900 border-l border-blue-500/30 flex items-center justify-center"
                        style="transform: rotateY(-90deg)"
                    ></div>
                    <!-- 缩略图顶部 -->
                    <div
                        class="absolute top-0 left-0 right-0 h-2.5 origin-top bg-blue-700"
                        style="transform: rotateX(90deg)"
                    ></div>
                    <!-- 缩略图正面 -->
                    <div
                        class="absolute inset-0 bg-blue-500 p-px z-10"
                    ...
                    >
                        ...
                        ></div>
                        <div
                            class="bg-black flex flex-col h-full w-full overflow-hidden"
                        >
                            <div
            ...
            <div class="flex-1 min-w-0 pr-2">
                <div class="flex items-center gap-2 mb-1">
                    <span class="text-gray-400 text-sm font-mono font-medium"
                        >#{{ formattedIndex }}</span
                    >
                    <h3
                        class="text-gray-800 font-bold text-lg truncate group-hover:text-blue-600 transition-colors"
                    >
                        {{ entry.title }}
                    </h3>
                </div>
                ...
                <p
                    v-if="entry.originalTitle"
                    class="text-gray-500 text-sm mb-2 truncate"
                >
                    {{ entry.originalTitle }}
                </p>
                ...
                <div class="flex flex-wrap items-center gap-2">
                    <span
                        class="text-xs px-2 py-0.5 rounded-sm text-white shadow-sm"
                        :style="{
                    ...
                    <span
                        v-if="entry.year"
                        class="text-gray-400 text-sm ml-auto"
                    >
                        {{ entry.year }}
                    </span>
                </div>
            ...
            <div class="hidden sm:block shrink-0 text-right w-24">
                <div
                    v-if="entry.rating > 0"
                    class="text-yellow-500 text-sm mb-1 tracking-widest"
                >
                    {{ getRatingStars(entry.rating) }}
                </div>
                <div
                    class="w-full h-1.5 bg-gray-100 rounded-full overflow-hidden mt-2"
                >
                    <div
                        class="h-full transition-all duration-500 rounded-full"
                        :style="{
                            backgroundColor: categoryInfo?.color || '#3B82F6',
                            width: `${(entry.rating / 5) * 100}%`,
                        }"
                    />
                </div>
            </div>
...
const categoryInfo = computed(() =>
    dexCategories.find((category) => category.id === props.entry.category),
);
const statusInfo = computed(() =>
    dexStatuses.find((status) => status.id === props.entry.status),
);
