            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8 flex-1">
                ...
                <div class="text-center">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="pixel-box bg-white/80 rounded p-3 relative overflow-hidden">
                            <div class="text-lg relative z-10">{{ totalPostsCount }}
                            </div>
                            <div class="relative z-10">总文章数</div>
                        </div>
                        <div class="pixel-box bg-white/80 rounded p-3 relative overflow-hidden">
                            <div class="text-lg relative z-10">{{ totalWordCount }}
                            </div>
                            <div class="relative z-10">总字数</div>
                        </div>
                        <div class="pixel-box bg-white/80 rounded p-3 relative overflow-hidden">
                            <div class="text-lg relative z-10">{{ totalViewsCount }}
                            </div>
                            <div class="relative z-10">总阅览次数</div>
                        </div>
                        <div class="pixel-box bg-white/80 rounded p-3 relative overflow-hidden">
                            <div class="text-lg relative z-10">{{ shortUptime }}</div>
                    ...
                    <div class="flex justify-center md:justify-end gap-3">
                        <div v-for="socialLink in socialLinks" :key="socialLink.name">
                            <a :href=socialLink.url target="_blank" class="text-blue-500 hover:text-blue-700">
                                <div class="pixel-box p-1! w-12 h-12 shrink-0" v-html="socialLink.icon"></div>
                                {{ socialLink.name }}
                            </a>
                        </div>
                    </div>
...
const { totalPostsCount, totalWordCount, totalViewsCount } = storeToRefs(postStore)
