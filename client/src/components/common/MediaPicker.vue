...
function selectMedia(media: MediaResponse) {
    selectedPath.value =
        selectedPath.value === media.filePath ? "" : media.filePath;
}
...
<template>
    ...
    <div class="space-y-2">
        <div
            v-if="modelValue"
            class="relative w-48 h-32 border-4 border-black overflow-hidden"
        >
            <img
                :src="modelValue"
        ...
        <button
            type="button"
            @click="openPicker"
            class="pixel-btn bg-sky text-sm"
        >
    ...
    <Teleport to="body">
        ...
        >
            <div
                class="bg-white border-4 border-black w-full max-w-4xl max-h-[85vh] flex flex-col pixel-card"
            >
                <!-- 头部 -->
                <div
                    class="p-4 border-b-4 border-black flex items-center justify-between"
                >
                    <h2 class="pixel-text text-lg">🖼️ 选择封面图片</h2>
                    <div class="flex items-center gap-3">
                        <!-- 上传新图片 -->
                        <label
                            class="pixel-btn bg-green-400 text-sm cursor-pointer"
                        >
                            📤 上传新图片
                            <input
                                type="file"
                        ...
                        <span v-if="uploading" class="text-sm text-blue-600"
                            >⏳ 上传中...</span
                        >
                    </div>
                </div>
                ...
                <div class="flex-1 overflow-y-auto p-4">
                    <div
                        ...
                        class="text-center py-10 text-gray-400 pixel-text"
                    >
                        媒体库中还没有图片，请先上传 🖼️
                    </div>
                    <div
