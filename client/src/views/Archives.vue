    ...
    return postStore.archives
        .map(am => ({
            ...
            posts: am.posts.filter(post => {
                // ① 日期范围
                if (dateFrom.value && new Date(post.createdAt) < new Date(dateFrom.value)) return false
                if (dateTo.value) {
                ...
                // ② 分类
                if (selectedCategory.value !== 'all' && post.category?.slug !== selectedCategory.value) return false
                // ③ 标签（多选，满足任一）
                if (selectedTags.value.length > 0 && !post.tags.some(t => selectedTags.value.includes(t.slug))) return false
                return true
            }),
        }))
...
const groupedPosts = computed(() => {
    const groups: { [year: number]: { year: number; month: number; posts: typeof postStore.archives[0]['posts'] }[] } = {}
                ...
                <div class=" space-y-4 p-4 pixel-card pixel-box  bg-yellow-100!">
                    ...
                    <div>
                        ...
                        <div class="flex flex-wrap gap-2">
                            ...
                            <button v-for="cat in allCategories" :key="cat.slug" :class="selectedCategory === cat.slug
                                ? '  bg-sky-dark text-white text-sm'
                                : '  hover:bg-black/5'"
                                @click="selectedCategory = selectedCategory === cat.slug ? 'all' : cat.slug">
                                <PixelBadge :name="cat.name" :color="cat.color" :icon="cat.icon"></PixelBadge>
                            </button>
                            <span v-if="allCategories.length === 0" class="  ">（暂无可筛选分类）</span>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="flex flex-wrap gap-2">
                            <button v-for="tag in allTags" :key="tag.slug" :class="selectedTags.includes(tag.slug)
                                ? 'bg-sky-light/60  border-sky-dark text-sm'
                                : '  border-transparent hover:bg-black/5'" @click="selectedTags = selectedTags.includes(tag.slug)
                                    ? selectedTags.filter(s => s !== tag.slug)
                                    : [...selectedTags, tag.slug]">
                                <PixelBadge :name="tag.name" :color="tag.color"></PixelBadge>
                            </button>
                            <span v-if="allTags.length === 0" class="  ">（暂无可筛选标签）</span>
                        </div>
                    </div>
