...
watch(
  ...
  (newPath) => {
    if (newPath) {
      const pathParam = Array.isArray(newPath) ? newPath.join('/') : newPath
      loadNote(decodeURIComponent(pathParam))
    } else {
      // 回到了 /notes，清空内容
    ...
    <div class="notes-page">
      ...
      <aside class="notes-sidebar">
        ...
        <div v-else class="sidebar-tree">
          <FileTree :nodes="treeNodes" :active-path="activeFilePath" :level="0" @select="onFileSelect" />
        </div>
      </aside>
      ...
      <main class="notes-content">
        <!-- 未选择任何笔记时显示欢迎页 -->
        <div v-if="!activeFilePath && !contentLoading" class="welcome-screen">
          <div class="welcome-card">
            ...
            <div class="welcome-stats">
              ...
              <div class="stat-item">
                <span class="stat-number">{{ treeNodes.length ? 'GitHub' : '—' }}</span>
        ...
        <!-- 内容加载失败 -->
        <div v-else-if="contentError" class="content-status content-error">
          <span>❌ {{ contentError }}</span>
          <button class="retry-btn" @click="loadNote(activeFilePath!)">重试</button>
        </div>
        ...
        <article v-else-if="noteContent" class="note-article">
          ...
          <div class="note-breadcrumb">
            <router-link to="/notes" class="breadcrumb-link">笔记</router-link>
            <span v-if="currentFolderPath" class="breadcrumb-sep">/</span>
            <span v-if="currentFolderPath" class="breadcrumb-folder">{{ currentFolderPath }}</span>
            <span class="breadcrumb-sep">/</span>
            <span class="breadcrumb-current">{{ currentFileName }}</span>
          </div>

          <!-- Markdown 渲染内容 -->
          <div class="note-body">
            <div class="prose prose-lg max-w-none note-content" v-html="renderedContent" @click="handleContentClick" />
          </div>
        </article>
      </main>
    </div>
