            ...
            <div class="max-w-6xl mx-auto px-4 relative z-10">
                <div class="grid grid-cols-3 md:grid-cols-7 gap-3">
                    <div
                        ...
                        :key="cat.id"
                        class="text-center p-3 cursor-pointer transition-all border-4"
                        :class="
                    ...
                    >
                        ...
                        <div
                            class="pixel-text text-xs"
                            :style="{ color: cat.color }"
                        >
                            {{ cat.name }}
                        </div>
                        <div class="text-xs mt-1">
                            {{ dexStats.byCategory[cat.slug] || 0 }}
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="max-w-6xl mx-auto px-4">
                ...
                >
                    ...
                    <div class="flex items-center gap-2">
                        <button
                            class="p-2 border-4 transition-all"
                            :class="
                                viewMode === 'grid' ? 'bg-gold' : 'bg-white '
                            "
                            @click="viewMode = 'grid'"
                        >
                            <svg
                        ...
                        <button
                            class="p-2 border-4 transition-all"
                            :class="
                                viewMode === 'list' ? 'bg-gold' : 'bg-white '
                            "
                            @click="viewMode = 'list'"
                        >
                            <svg
                ...
                <div class="flex items-center gap-4 mb-6 p-4 bg-white border-4">
                    <div class="flex items-center gap-2">
                        ...
                        <span class="pixel-text text-lg text-sky-dark">{{
                            dexStats.total
                        }}</span>
                    </div>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <span class="pixel-text text-lg text-gold-dark">{{
                            dexStats.averageRating
                        }}</span>
                    </div>
                    <div class="w-px h-6"></div>
                    <div class="flex items-center gap-2">
                        <span class="pixel-text text-xs">已完成:</span>
                        <span class="pixel-text text-lg text-grass-dark">{{
                            dexStats.byStatus.completed
                        }}</span>
                    </div>
                </div>
                ...
                <div
                    v-if="!searchQuery && hasMore && dexs.length > 0"
