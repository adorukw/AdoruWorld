...
import type { MediaTagResponse } from '@/types'

const store = useMediaTagStore()
const items = ref<MediaTagResponse[] | null>(null)

const showModal = ref(false)
const modalMode = ref<'create' | 'update'>('create')
const editingItem = ref<MediaTagResponse | null>(null)

onMounted(async () => {
    await store.getMediaTags()
    items.value = store.mediaTags
})
...
async function handleDelete(item: any) {
    ...
    try {
        await store.deleteMediaTag(item.id)
        await store.getMediaTags()
        items.value = store.mediaTags
    } catch (e) {
        alert('删除失败')
...
<template>
    ...
    <CrudModal v-if="showModal" module="mediaTags" :mode="modalMode" :initial-data="editingItem"
