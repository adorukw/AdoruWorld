    ...
    <section class="pb-12">
      <div class="max-w-6xl mx-auto px-4">
          ...
          >
            ...
            <div class="flex items-start justify-between gap-2 mb-2">
              <a :href="repo.html_url" target="_blank"
                class="pixel-text text-sm text-[#3b4cca] hover:text-[#4a5fd8] truncate
            ...
            <div v-if="repo.topics && repo.topics.length" class="flex flex-wrap gap-1.5 mt-3">
              <span v-for="topic in repo.topics.slice(0, 4)" :key="topic"
                class="px-2 py-0.5 text-[10px] leading-none
                  border border-black/30 bg-[#e8f0fe] text-[#1967d2]"
              >
                {{ topic }}
              </span>
              <span v-if="repo.topics.length > 4"
        ...
        <div v-else class="pixel-card p-8 text-center">
          ...
          <PixelButton color="blue" @click="window.location.reload()">
            重试
          </PixelButton>
        </div>
      </div>
