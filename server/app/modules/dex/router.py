...
async def get_dex_stats(db: Annotated[AsyncSession, Depends(get_db)]):
    return await crud.get_dex_stats(db)


@router.get("/slug/{slug}", response_model=DexResponse)
async def get_dex_by_slug(slug: str, db: Annotated[AsyncSession, Depends(get_db)]):
...
async def get_related_dex(slug: str, db: Annotated[AsyncSession, Depends(get_db)]):
...
async def get_dex_by_id(entry_id: str, db: Annotated[AsyncSession, Depends(get_db)]):
